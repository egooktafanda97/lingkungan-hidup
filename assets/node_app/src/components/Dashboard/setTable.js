...
export const Colom = (data) => {
  const ItemSet = {
    selectFilter: ["nama", "nama_pemilik", "nama_usaha", "zona"],
    Column: [
      {
        title: "No",
        style: {
...
export const Build = (data, response, action) => {
  ...
  data.map((item, i) => {
    result.push({
      ...
      alamat: item.alamat,
      Jenis_Usaha: item.jenis_usaha.nama_jenis_usaha,
      Jumlah_Retribusi: rupiah(item.jenis_usaha.jumlah_retribusi, "Rp"),
      action: (
        <div
          className='d-flex'
