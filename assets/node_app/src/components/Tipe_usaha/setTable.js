...
export const Build = (data, response, action) => {
	...
	data.map((item, i) => {
		result.push({
			...
			sumber: item.tipe_sumber_sampah,
			keterangan_sampah: item.keterangan_sampah,
			jumlah_retribusi: rupiah(item.jumlah_retribusi, "Rp"),
			...
			action: (
				...
				>
					<DropdownMenu
						...
						onClickDelete={() => {
							action.onDelete(item.id_tipe_usaha);
						}}
					/>
				</div>
			),
